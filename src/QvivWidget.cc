                        ...
                        const QString& distString)
{
    ...
    painter->fillRect(int(-dist/2-margin), -20-margin,
                      int(dist+2*margin), 20+2*margin, color);
...
public:
    ...
    void reset()
    {
        this->x0=this->y0=-HUGE_VAL;
        this->x1=this->y1=-HUGE_VAL;
    }
...
public:
    QLabel* w_balloon;
    QImage label_image;
    shared_ptr<QvivData> qviv_data;
    shared_ptr<QvivData> qviv_measure_data;
    ...
    double last_x, last_y;     // Last measuring coord in canvas coordinates
...
{
    //abort_pick_point();      // Is this needed?
    if (d->event_loop)
      d->event_loop->exit(-999);
    delete d;
}
...
void QvivWidget::set_qviv_data(shared_ptr<QvivData> qviv_data)
{
                               ...
                               double scale_x, double scale_y)
{
    ...
    if (d->do_show_marks && d->qviv_data)
    {
...
void QvivWidget::mouseMoveEvent (QMouseEvent *event)
{
    if (d->do_show_balloon)
    {
      ...
      if (label_color >= 0xff000000 && label > 0 && d->qviv_data)
      {
                           ...
                           )
{
    ...
    if (ret==-999)  // Indication that the widget was destroyed. Don't touch
                    // Member variables!!
      return ret;
